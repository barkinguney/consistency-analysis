...
GUM-consistent recipe to turn Type-B systematic term $\delta$ into a combined uncertainty $u_c(y)$, eq. (1), with Type-A random. So, calculated in this section $\delta$ tis systematic expanded relative uncertainty will be later transformed in the systematic standard uncertainty.
...
To define and $\xi_i$, we use a two-sided, smaller-is-better modification of Harrington's desirability concept [48] [49] retaining low sensitivity near the ideal point and saturation for large deviations (Fig. 4):
\begin{equation}
	\xi_i = 1 - e^{-(x_i^2)}
\end{equation}
\begin{equation}
...
For binary IFs (e.g., tailoring, inserts, CRV, dilution, impurities), we map an unfavorable setting to the conservative lower edge of the ``satisfactory'' band, grey color in Fig. 4, i.e. $\xi_i = e^{-1} \cong 0.37$, if ``no'' and to $\xi_i=0$ if ``yes'' for $\text{IF}_{4,5,6,10,13}$. For $\text{IF}_{11}$ it is otherwise. 

As the test time ($\text{IF}_9$) is strongly correlated with $\text{T}_5$ and $\text{P}_5$, we retain $\text{IF}_9$ in the systematic term and exclude $\text{T}_5$ and $\text{P}_5$ from $\delta$ (their uncertainties enter the random part later). We also exclude $\text{IF}_{14}$ ($\text{P}_1$) due to a lack of quantitative evidence.
...
We apply eqs. (4) and (5) under assumptions $A, B, C, D, T_5, P_5$, and $\varphi$ are not correlated parameters and terms with parameter correlations can be neglected.
...
\end{equation}
$\sigma_{\delta_i}^2$ is the standard systematic deviation. As we interpret $\delta$ as normal, 95\% expanded, we obtain:
\begin{equation}

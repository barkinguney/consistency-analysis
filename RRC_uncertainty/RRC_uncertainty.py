...
def save_plot_fit_results(T, k, source, fit_result, output_dir, reaction_name=None, k_band=None):
    ...
    for s, values in grouped.items():
        ax.semilogy(values['T'], values['k'], '-', label=s, alpha=0.95)

    ax.semilogy(T_plot, k_fit, '-', linewidth=2.5, label='Best-fit Arrhenius model')
    ax.fill_between(T_plot, k_lower, k_upper, color='gray', alpha=0.5, label='[k_lower, k_upper]')
    ax.set_xlabel('1000/T [1/K]')
    ax.set_ylabel('Rate Coefficient [m^3/(mol*s)]')
    if reaction_name is None:
        title_text = f' f =[{np.min(uncertainty_factor):.3f} - {np.max(uncertainty_factor):.3f}]'
    else:
        title_text = (
            f'{reaction_name}\n'
            f'f =[{np.min(uncertainty_factor):.3f} - {np.max(uncertainty_factor):.3f}]'
        )
...
if __name__ == "__main__":
    ...
    input_dir = Path("RRC_uncertainty/results/rate_coefficients")
    out_dir = Path("RRC_uncertainty/results")
    out_dir_plot = Path("RRC_uncertainty/results/fit_plots")
    out_dir_fit = Path("RRC_uncertainty/results/ls_fits")
    ...
    for file in input_dir.glob("*.csv"):
        ...
        save_plot_fit_results(T, k, source, ls_fit, plot_path, reaction_name=rate_coeff_df['Reaction'].iloc[0])
